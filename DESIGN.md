...
### Motion
- **Hero light beams** (`hero-beams.tsx`, `globals.css` `beam-drift`): two blue gradient beams behind the hero copy drift slowly and independently (14s/18s, opposite direction) — purely ambient, `aria-hidden`, `pointer-events: none`. Pure CSS; no `prefers-reduced-motion` handling in the component itself, since the global reduced-motion rule in `globals.css` already collapses any animation's duration/iteration-count regardless of where it's declared.
...
- **Portfolio card hover:** the image scales to `1.04` (clipped by the card's own rounded corners) and the lit dot scales to `1.25` — reinforcing the card as a selectable channel.
