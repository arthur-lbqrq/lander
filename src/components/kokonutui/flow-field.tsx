 ...
 *
 * LOCAL ADDITIONS (lander.co, beyond the upstream original — see DESIGN.md
 * "Flow Field"): a fourth theme, "signal", locked to this project's exact
 * Signal Blue accent with no hue drift, plus two new per-theme knobs
 * (`hueJitter`, `maxAlpha`) that make a genuinely single-tone, more discreet
 * theme possible. The three original themes (aurora/ember/ocean) keep their
 * exact original look — the new knobs default to the original hardcoded
 * values (70 and 0.9) wherever a theme doesn't set them.
 */
...
type ColorTheme = "aurora" | "ember" | "ocean" | "signal";
...
interface ThemeConfig {
  ...
  bg: string;
  trailAlpha: number;
  /** Max degrees the flow angle drifts a particle's hue away from hueStart. Local addition; original behavior is 70 for every theme. */
  hueJitter?: number;
  /** Peak particle opacity (before the lifetime fade-in/out). Local addition; original behavior is 0.9 for every theme. */
  maxAlpha?: number;
}
...
const THEMES: Record<ColorTheme, ThemeConfig> = {
  ...
  /**
   * Local addition. Locked to lander.co's own Signal Blue accent
   * (#2d5cff ≈ hsl(227, 100%, 59%) — DESIGN.md "Colors") rather than a
   * hue range: hueRange 0 and hueJitter 0 mean every particle renders at
   * exactly this one hue, never drifting into cyan/violet like the other
   * themes do. bg is the system's own Near-Black Ink (#0a0a0a = rgb(10,10,10)),
   * not an approximation. maxAlpha is lowered for a genuinely more discreet
   * presence, and trailAlpha is raised slightly for shorter, quieter trails.
   */
  signal: {
    hueStart: 227,
    hueRange: 0,
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (const p of particles) {
        ...
        const alpha = fadeIn * fadeOut * (cfg.maxAlpha ?? 0.9);

        // Hue shifts subtly with field direction for color variety
        // (hueJitter 0 — e.g. the "signal" theme — means no shift at all).
        const hueMod = (p.hue + (angle / (Math.PI * 2)) * (cfg.hueJitter ?? 70) + 360) % 360;
